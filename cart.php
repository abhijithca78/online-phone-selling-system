  ...
  <form action="razorpay\checkout.php?id=<?php echo $uid ?>" method="POST">
    <table>
      ...
      <?php
                ...
                $records = mysqli_query($conn,"SELECT cart.*, products.* FROM cart, products WHERE cart.phone_id = products.phoneid and cart.userid='$uid'"); // fetch data from database
                $totalrun=mysqli_query($conn,"SELECT sum(products.phone_price) as total_sum FROM cart, products WHERE cart.phone_id = products.phoneid and cart.userid='$uid'");
      ...
      <tr>
        <td>
          <div class="cart-info">
            ...
            <div>
              <p><?php echo $data['phone_name']; ?></p>
              <span></span>
              <br />
              <a href="cartremove.php?phoneid=<?php echo $data['phoneid']; ?>&uid=<?php echo $uid ?>">remove</a>
            </div>
          </div>
        </td>
        <td><?php echo $data['phone_price']; 
        $a = $data['phone_price'];?><input type="hidden" name="pr" class="iprice" id="pr" value="<?php echo $data['phone_price'] ?>">
        <td>
        <input type="hidden" name="uid" class="uid" id="uid" value="<?php echo $uid ?>">
        <input type="hidden" name="bid" class="bid" id="bid" value="<?php echo $data['phoneid']; ?>">
        <td>
        <input name='iquantity' id='iquantity' required type="number" class= 'iquantity'  onchange='subtotal()' name="no" min="0" max="<?php echo $data['phone_qty']; ?>" >
        </td>
        <td class = 'itotal'></td>
      </tr>
      <?php
    ...
    <div class="total-price">
      ...
      <button type="submit" class="checkout btn submit" name="submit" id="submit">Proceed To Checkout</button>
