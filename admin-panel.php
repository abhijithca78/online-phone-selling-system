...
$phoneprice=0;
 ...
 $frequent = mysqli_query($conn,"SELECT `phone_id` from `my_orders` group by `phone_id` order by count(*)desc limit 1;");
 while($mostfrequent = mysqli_fetch_array($frequent))
 {
    $mostfrequentid=$mostfrequent['phone_id'];
 }

 $mostselledphone= mysqli_query($conn,"SELECT * FROM `products` WHERE `phoneid`= '$mostfrequentid';");
 while($mostselledphonedetails = mysqli_fetch_array($mostselledphone))
 {
    $mostselledphonename=$mostselledphonedetails['phone_name'];
    $mostselledphoneprice=$mostselledphonedetails['phone_price'];
    $mostselledphonecategory=$mostselledphonedetails['category'];
    $mostselledphoneimage=$mostselledphonedetails['phone_image'];
 }

 $pricecalc= mysqli_query($conn,"SELECT `qty` FROM `my_orders` WHERE `phone_id` = '$mostfrequentid';");
 while($pricecalcdetails=mysqli_fetch_array($pricecalc))
 {
    $phoneqty= $pricecalcdetails['qty'];
    $phoneprice = $phoneprice + ($phoneqty * $mostselledphoneprice);
 }
?>
                        ...
                        <div class="box-body">
                            <ul class="product-list">
                                <li class="product-list-item">
                                    <div class="item-info">
                                        <img src="phone_image\<?php echo $mostselledphoneimage ?>" alt="product image">
                                        <div class="item-name">
                                            <div class="product-name"><?php echo $mostselledphonename ?></div>
                                            <div class="text-second"><?php echo $mostselledphonecategory ?></div>
                                        </div>
                                    </div>
                                    <div class="item-sale-info">
                                        <div class="text-second">Sales</div>
                                        <div class="product-sales">₹<?php echo $phoneprice ?></div>
                                    </div>
                                </li>
                            </ul>
                        </div>
