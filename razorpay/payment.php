<?php
include "C:/xampp/htdocs/omss/dbconn.php";
...
$query1 = "INSERT INTO `orders`() SELECT * FROM `razorpay` WHERE pay_id = '$pay_id';";
        ...
        while($data = mysqli_fetch_assoc($query3run))
        {
            $bid=$data['phone_id'];
            $qty=$data['qty'];
            $query4="SELECT * FROM `products` WHERE `phoneid`='$bid'";
            $query4run=mysqli_query($conn, $query4);
            $data1 = mysqli_fetch_assoc($query4run);
            
                $oldqty=$data1['phone_qty'];
                $newqty= $oldqty - $qty;
                
                $query5="UPDATE `products` SET `phone_qty`='$newqty' WHERE `phoneid`= '$bid'";
...
if(isset($_POST['qty']))
{
    ...
    $query = "UPDATE `cart` SET `qty`='$qty' WHERE `phone_id`= $pid && `userid`=$uid";
